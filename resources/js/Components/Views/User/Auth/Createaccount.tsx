...
export default function Createaccount()
{
    ...
    const [credentials, setCredentials] = useState<data>({name: '', email: '', user_type: '', password: '', password_confirmation: ''});
    ...
    const updateCredentials2 = (e:ChangeEvent<HTMLSelectElement>) => {
        setCredentials({...credentials, [e.target.name] : e.target.value});
    }
        ...
        <div className="w-full flex flex-col min-h-screen p-5 md:p-10">
            ...
            <div className="w-full m-auto max-w-sm p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8">
                ...
                <form className="max-w-sm mx-auto" onSubmit={createaccount}>
                    ...
                    <div className="mb-5">
                        <label htmlFor="email" className="block mb-2 text-sm font-medium text-gray-900">Account type</label>
                        <select name="user_type" id="user_type" onChange={updateCredentials2} className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                            <option value=""></option>
